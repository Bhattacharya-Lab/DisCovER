...
my $help="
a3m2mtx.pl 
convert MSA to PSI-BLAST profile in MTX and PSSM format.
...
Usage: a3m2mtx.pl <ali_file> [<outbase(outfile)>] [-fas|-a3m|-clu|-sto|-aln] [-neff 7]
\n";
...
&HHPaths::System("cp $tmpfile.mtx $outfile.mtx");
&HHPaths::System("cp $tmpfile.pssm $outfile.pssm");
&HHPaths::System("cp $tmpfile.chk $outfile.chk");
...
sub RunPseudoPsiblast() {
    ...
    &HHPaths::System("$ncbidir/blastpgp -b 1 -j 1 -h 0.001 -d $dummydb -i $infile -B $tmpfile.in.psi -C $tmpfile.chk -Q $tmpfile.pssm -o $tmpfile.blastout 1> $tmpfile.blalog 2> $tmpfile.blalog");
